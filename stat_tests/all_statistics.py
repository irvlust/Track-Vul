...
from app.schemas import AllDependenciesCreate
from fastapi import HTTPException, status
from app.database import get_db
from app.models import AllDependencies as DBDepCreate
...
async def create_dependency(
    ...
    raw_data = {
        "application_name": f"Application name{str(app_number)}",
    ...
    try:
        dep_data = AllDependenciesCreate.model_validate(raw_data)
    ...
    dep_obj = DBDepCreate(**dep_data.model_dump())
    db.add(dep_obj)
...
async def parse_content(content, app_number: int, db: AsyncSession):
    ...
    for line in content.splitlines():
        ...
        try:
            ...
            await create_dependency(req.name, specs, extras, app_number, db=db)
